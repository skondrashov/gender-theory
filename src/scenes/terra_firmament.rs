...
pub const SCENES: &[&Scene] = &[
	&Scene {
		...
		render: &|draw, model, i, j, time, whole| {
			...
			let mut gray = half * noise * noise2 * 3.0 + 0.2 * (quarter - 1.0).abs() + 0.3 * noise2;
	...
	&Scene {
		...
		render: &|draw, model, i, j, time, whole| {
			...
			let (i, j) = scale_coordinates(model, i, j);
			let mut gray = half * noise * noise * noise * noise2 * 3.0 + 0.3 * noise2;
			let (i0, i1) = (i * 1.00 + noise2, (i + 20.0 * noise) + eighth);
			let (j0, j1) = (
				j - 12.0 * (quarter * 3.0).sin() - noise2 * 10.0,
				(j + 3.0 * noise.cos() * eighth * (eighth * 2.0).sin()),
			);
